...
include($_SERVER["DOCUMENT_ROOT"]."/include/content_docum_icon.php");
?>


<script src='<?=SITE_TEMPLATE_PATH?>/js/galleria-1.4.2.min.js'></script>
<link rel="stylesheet" href="<?=SITE_TEMPLATE_PATH?>/js/galleria.classic.css">
        ...
        <div class="nomer_pict">
          ...
          <div class="gal_price text-center">
          	...
          	<div>
          		...
          		<div><?=number_format($arResult['PROPERTIES']['PRICE']['VALUE'], 0, '', ' ' )?> <i class="fa fa-rub"></i></div>
          		<?endif?>
          		<a href=# data-toggle="modal" data-target="#callback_bron_id"  class="btn btn-default"><?=GetMessage('botton_zakaz')?></a>
          	</div>
         </div>
          ...
          <div id="galleria">
                ...
                $big_pict = $arResult['DETAIL_PICTURE']['SRC'];
                ?>
         ...
         <div class="galleria-layer" href="<?=$arResult["DETAIL_PICTURE"]['SRC']?>"  style="position: absolute; top: 0px; left: 0px; right: 0px; bottom: 0px; z-index: 3;"  >wewer
         </div>
		...
		<?
		vopros_button($arResult['DETAIL_TEXT']);
		?>
		<?endif?>
		...
		<h2 class="title_content_more"><i class="fa fa-folder-open-o"></i> <?=GetMessage('PROJECTS_TITLE')?></h2>
...
<script src="<?=SITE_TEMPLATE_PATH?>/js/blueimp-gallery-indicator.js"></script>
    ...
    Galleria.loadTheme('<?=SITE_TEMPLATE_PATH?>/js/galleria.classic.min.js');
    // Initialize Galleria
