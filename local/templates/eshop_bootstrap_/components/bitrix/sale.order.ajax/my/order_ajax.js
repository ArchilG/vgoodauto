	...
	BX.Sale.OrderAjaxComponent = {
		...
		getBlockFooter: function(node)
		{
			...
			if (currentSection && currentSection.id.indexOf(firstSection.id) == '-1')
			{
				buttons.push(
					BX.create('A', {
						props: {
							href: '',
							className: 'pull-left btn btn-danger btn-md'
						},
			...
			if (!isLastNode)
			{
				buttons.push(
					BX.create('A', {
						props: {href: '', className: 'pull-right btn btn-danger btn-md'},
		...
		editSocialContent: function(authContent)
		{
			...
			nodes.push(BX.create('DIV', {
				...
				children: [
					...
					BX.create('A', {
						props: {className: 'btn btn-danger btn-lg'},
				...
				for (i = 0; i < activeStores.length; i++)
				{
					...
					geoObj = new ymaps.Placemark([activeStores[i].GPS_N, activeStores[i].GPS_S], {
						...
						balloonContentLayout: ymaps.templateLayoutFactory.createClass(
							...
							'<br /><a class="btn btn-sm btn-danger" data-store="{{ properties.id }}">{{ properties.text }}</a>',
							{
								build: function() {
			...
			storeNode = BX.create('DIV', {
				...
				children: [
					...
					BX.create('DIV', {
						...
						children: [
							BX.create('A', {
								props: {href: '', className: 'btn btn-sm btn-danger'},
								text: this.params.MESS_SELECT_PICKUP,
								events: {
					...
					if (property.isMultiple())
					{
						propContainer.appendChild(
							BX.create('DIV', {
								...
								props: {className: 'btn btn-sm btn-danger'},
		...
		alterProperty: function(settings, propContainer)
		{
			...
			{
				BX.addClass(add[i], 'btn btn-danger btn-sm');
			...
			if (add.length)
			{
				...
				BX.bind(add, 'click', BX.proxy(function(e){
					...
					if (add.length && add[add.length - 2])
					{
						...
						BX.addClass(add[add.length - 2], 'btn btn-danger btn-sm');
					}
						...
						children: [
							BX.create('A', {
								props: {
									...
									className: 'btn btn-danger btn-lg btn-order-save'
								},
