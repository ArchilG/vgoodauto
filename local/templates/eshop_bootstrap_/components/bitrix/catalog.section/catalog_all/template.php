...
<?else:?>
	...
	<?if($APPLICATION->GetCurPage(false) == SITE_DIR || $arParams['FILTER_NAME'] == 'arrFilter_recom_datail'):?>
		<?if ($_SESSION['arr_set']['type_menu'] == 'v' && $arParams['FILTER_NAME'] !== 'arrFilter_recom_datail' ):?>
...
<?endif?>
	...
	<div class="list_prod" id="<? echo $strMainID; ?>" <?if($APPLICATION->GetCurPage(false) == SITE_DIR):?> style="background: none; "<?else:?><?endif?>>
	...
	<?// endif?>
		...
		<?if($APPLICATION->GetCurPage(false) !== SITE_DIR): ?>
		<a data-toggle="popover<?=$arItem['ID']?>" id="<? echo $arItemIDs['PICT']; ?>" href="<? echo $arItem['DETAIL_PAGE_URL']; ?>" title="<? echo $imgTitle; ?>">
		<?else:?>
		...
		if($arItem['PREVIEW_PICTURE']['SRC'])
			echo	'
		<div class="b_red1" style="background: url('.$arItem['PREVIEW_PICTURE']['SRC'].') no-repeat center center; background-size: contain; height: 250px; width: auto; margin-bottom: 20px; margin-top: 10px;">
		</div>';
		else
			echo	'
		<div class="b_red1" style="background: url('.$arItem['PREVIEW_PICTURE']['SRC'].') no-repeat center center; background-size: contain; height: 250px; width: auto; margin-bottom: 20px; margin-top: 10px;">
...
<div class="cena_nalichie">
	...
	<table width="100%">
		<tr>
			<td valign="middle">
			<!-- <div class="b_red" id="img<?=$arItem["ID"];?>" style="width: 100px; height: 100px;"></div> -->
			<div class="cena">
		 <div id="img<?=$arItem["ID"];?>">
...
<?
	...
	?>
			...
			<div class="v_nalichii"><i class="fa fa-check"></i> <?=GetMessage('SKLAD_YES')?>
			<?else:?>
...
<a id="<? echo $arItemIDs['QUANTITY_UP']; ?>" href="javascript:void(0)" class="bx_bt_button_type_2 bx_small" rel="nofollow"><i class="fa fa-plus"></i></a>
		...
		<div id="<? echo $arItemIDs['BASKET_ACTIONS']; ?>">
			<a id="<? echo $arItemIDs['BUY_LINK']; ?>" class="btn btn-danger  btn-sm car tt" href="javascript:void(0)" rel="nofollow">
			<?
...
<div class="btn-group" data-toggle="buttons" style="width: 100%" >
  ...
  <label class="btn <?=$active?>">
    <input style="display: none; padding: 0; margin: 0;" <?=$checked;?> type="checkbox" autocomplete="off" id="compareid_<?=$arItem['ID'];?>" onchange="compare_tov(<?=$arItem['ID'];?>,<?=$arItem['IBLOCK_ID']?>);"> <i class="fa fa-bar-chart"></i> <? echo $compareBtnMessage; ?>
  </label>
...
<?//$frame = $this->createFrame()->begin("");?>
  <label class="btn <?=$active?>">
    <input style="display: none;" checked type="checkbox" autocomplete="off" id="favid_<?=$arItem['ID'];?>" onchange="fav_tov(<?=$arItem['ID'];?>,<?=$arItem['IBLOCK_ID']?>);"><i class="fa fa-heart-o"></i> <?=GetMessage('ADD_FAV')?>
  </label>
  <?//$frame->end();?>
	</div> 
</div>
