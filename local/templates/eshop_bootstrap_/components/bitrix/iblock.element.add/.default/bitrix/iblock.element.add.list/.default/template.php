                        ...
                        <div class="panel-heading" role="tab" id="headingOne">
                            <a role="button" data-toggle="collapse" data-parent="#accordion" href="#collapse_<?=$arElement["ID"]?>" aria-expanded="false" aria-controls="collapse_<?=$arElement["ID"]?>" class="collapsed">
                                <h4 class="panel-title">
                                    <?=$arElement["NAME"]?>
                                    <sup><small>(<?=substr($arElement['DATE_CREATE'],0,-9)?>)</small></sup>

                                    <i class="fa fa-angle-up" aria-hidden="true"></i>
                                    <i class="fa fa-angle-down" aria-hidden="true"></i>

                                    <?if(!empty($arElement['PROPERTIES']['VIN']['VALUE'])):?><span class="label label-secondary copy_link pull-right mr20" data-content="<?=$arElement['PROPERTIES']['VIN']['VALUE']?>">Скопировать VIN</span><?endif?>
                                </h4>
                            </a>
                        </div>
                ...
                return this.each(function(){
                    $(this).on('keyup', function(e){
                        ...
                        if(search == '') {
                            ...
                            if($target.find('li:visible').size() === 0) {
                                var no_results = $('<li class="filterTable_no_results">Нет результата</li>')
                                $target.append(no_results);
                            }
                        }
                    });
                });
