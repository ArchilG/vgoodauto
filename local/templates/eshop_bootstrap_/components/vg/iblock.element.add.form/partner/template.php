...
//перемещаем описание в конец
$el = $arResult["PROPERTY_LIST"][1];
unset($arResult["PROPERTY_LIST"][1]);
$arResult["PROPERTY_LIST"][] = $el;
?>
...
<div class="modal-header">
    ...
    <h3><?=$arParams['ELEMENT_ID']?'Изменение партнёра №'.$arParams['ELEMENT_ID']:'Новый партнёр'?></h3>
</div>
				...
				<div class="form-group control-group mb0">
					...
					<div class="col-sm-9 controls">
						...
						switch ($INPUT_TYPE):
							...
							case "T":
								...
								?>
						<textarea class="form-control" cols="<?=$arResult["PROPERTY_LIST_FULL"][$propertyID]["COL_COUNT"]?>" rows="5" name="PROPERTY[<?=$propertyID?>][<?=$i?>]"><?=$value?></textarea>
								<?
								}
							break;
							...
							case "N":
								...
								?>
                                    ...
                                    ?>
                                    <input autocomplete="off" type="text" class="form-control <?=$propertyID == 237 ? 'phone-mask' : ''?>" name="PROPERTY[<?=$propertyID?>][<?=$i?>]" size="<?=$arResult["PROPERTY_LIST_FULL"][$propertyID]["COL_COUNT"]; ?>" value="<?=$value?>" /><?
					...
					[<a href="<?=$arResult["ELEMENT_FILES"][$value]["SRC"]?>"><?=GetMessage("IBLOCK_FORM_FILE_DOWNLOAD")?></a>]<br />
							...
							<input type="<?=$type?>" name="PROPERTY[<?=$propertyID?>]<?=$type == "checkbox" ? "[".$key."]" : ""?>" value="<?=$key?>" id="property_<?=$key?>"<?=$checked ? " checked=\"checked\"" : ""?> /><label for="property_<?=$key?>"><?=$arEnum["VALUE"]?></label><br />
									...
									case "multiselect":
									?>
							<select name="PROPERTY[<?=$propertyID?>]<?=$type=="multiselect" ? "[]\" size=\"".$arResult["PROPERTY_LIST_FULL"][$propertyID]["ROW_COUNT"]."\" multiple=\"multiple" : ""?>" class="form-control">
